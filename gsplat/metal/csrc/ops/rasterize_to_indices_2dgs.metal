...
inline uint batch_count_from_isect_range_2dgs(
...
inline bool contributes_to_pixel_2dgs(
    float px,
    float py,
    float3 xy_opac,
    float3 u_M,
    ...
    const float sigma = 0.5f * min(gauss_weight_3d, gauss_weight_2d);
...
kernel void rasterize_to_indices_2dgs_count_kernel(
    device const float* transmittances [[buffer(0)]],
    device const float* means2d [[buffer(1)]],
    device const float* ray_transforms [[buffer(2)]],
    ...
    const uint num_batches = batch_count_from_isect_range_2dgs(
...
kernel void rasterize_to_indices_2dgs_emit_kernel(
    device const float* transmittances [[buffer(0)]],
    device const float* means2d [[buffer(1)]],
    device const float* ray_transforms [[buffer(2)]],
    device const float* opacities [[buffer(3)]],
